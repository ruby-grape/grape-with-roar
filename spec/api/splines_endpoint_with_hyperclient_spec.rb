...
describe Acme::Api::SplinesEndpoint do
  ...
  context 'splines' do
    ...
    it 'returns all splines by default' do
      expect(client.splines({}).count).to eq 3
    end

    it 'returns all splines paginating by 2' do
      expect(client.splines(size: 2).count).to eq 3
    end

    it 'returns pagination' do
      response = client.splines(size: 1, page: 2)
      expect(response._links.next._url).to eq 'http://example.org/api/splines?page=3&size=1'
      expect(response._links.prev._url).to eq 'http://example.org/api/splines?page=1&size=1'
      expect(response._links.self._url).to eq 'http://example.org/api/splines?page=2&size=1'
    end
  ...
  context 'spline' do
    ...
    it 'creates a spline' do
      ...
      expect(spline.id).not_to be_blank
